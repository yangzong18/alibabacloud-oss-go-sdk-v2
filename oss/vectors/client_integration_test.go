...
var (
	...
	endpoint_   = os.Getenv("OSS_TEST_VECTORS_ENDPOINT")
...
func cleanBucket(bucketInfo VectorBucketProperties, t *testing.T) {
	...
	if strings.Contains(endpoint_, *bucketInfo.ExtranetEndpoint) ||
		...
		c = getClient(*bucketInfo.Region, strings.TrimPrefix(*bucketInfo.ExtranetEndpoint, "oss-"))
	}
	assert.NotNil(t, c)
...
func TestBucketLogging(t *testing.T) {
	...
	_, err = ossClient.PutBucket(context.TODO(), putOssRequest)
	assert.Nil(t, err)
	...
	var serr *oss.ServiceError
	bucketNameNotExist := "logging-bucket-not-exist"
	request = &PutBucketLoggingRequest{
		Bucket: oss.Ptr(bucketNameNotExist),
		BucketLoggingStatus: &BucketLoggingStatus{
			LoggingEnabled: &LoggingEnabled{
	...
	ossClient.DeleteBucket(context.TODO(), &oss.DeleteBucketRequest{
		Bucket: oss.Ptr(targetBucketName),
	})
}
